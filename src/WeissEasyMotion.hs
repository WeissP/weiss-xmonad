...
{- |
Module      :  WeissEasyMotion
Description :  Focus a visible window using a key chord. Overrides visible windows discovery using Utils.visibleWindows and Utils.visibleAllWindows.
...
This fork uses Utils.visibleWindows and Utils.visibleAllWindows to override visible window discovery.
-}
module WeissEasyMotion (
  -- * Usage
  -- $usage
...
import Data.Map.Strict qualified as M (Map, elems, map, mapWithKey)
import Utils (visibleAllWindows, visibleWindows)
import XMonad
import XMonad.Prelude
import XMonad.StackSet qualified as W
import XMonad.Util.Font (Align (AlignCenter), XMonadFont (..), initXMF, releaseXMF, textExtentsXMF)
import XMonad.Util.XUtils (createNewWindow, deleteWindow, paintAndWrite, showWindow)
...
data OverlayWindow
  = OverlayWindow
  { win :: !Window
  -- ^ The window managed by xmonad
  , attrs :: !WindowAttributes
  -- ^ Window attributes for @win@
  , overlay :: !Window
  -- ^ Our window used to display the overlay
  , rect :: !Rectangle
  -- ^ The rectangle of @overlay@
  }
...
data Overlay
  ...
  -- ^ The window managed by xmonad
  , chord :: ![KeySym]
  -- ^ The chord we'll display in the overlay
  }
...
data EasyMotionConfig
  ...
  , emFont :: !String
  -- ^ Font for selection characters (passed to 'initXMF')
  , borderPx :: !Int
...
-- | Handles overlay display and window selection. Determines visible windows using Utils.visibleWindows and Utils.visibleAllWindows. Called after config has been sanitised.
...
handleSelectWindow c = do
  ...
  overlays :: [Overlay] <- case sKeys c of
    AnyKeys ks -> buildOverlays ks <$> sortedOverlayWindows
      where
        ...
        sortedOverlayWindows = visibleAllWindows >>= (fmap sortOverlayWindows . buildOverlayWindows th)
    PerScreenKeys m ->
      fmap concat $
        sequence $
          M.elems $
      ...
      where
        ...
        sortedOverlayWindows sid = visibleWindows sid >>= (fmap sortOverlayWindows . buildOverlayWindows th)
  ...
  where
    ...
    buildOverlayWindows th =
      fmap (fromMaybe [] . sequenceA)
...
handleKeyboard dpy drawFn cancel selected deselected = do
  ...
  ev <- io $ allocaXEvent $ \e -> do
    ...
    getEvent e
  if
    | ev_event_type ev == keyPress -> do
        s <- io $ keycodeToKeysym dpy (ev_keycode ev) 0
        if
          | s == cancel -> return Exit
