module WeissNamedScratchpad where
-- waitRun 10000 conf
...
data NamedScratchpad = NS
  ...
  , after :: Window -> X ()
  -- ^ this function will be called after the scratchpad is shifted to the current workspace
  }
...
existsNsp nsp = withWindowSet $ \winSet ->
  isJust
    <$> findNspFromWindows
      nsp
...
findNspCurrentWsp nsp = withWindowSet $ \winSet ->
  findNspFromWindows nsp $
    W.integrate' (W.stack . W.workspace . W.current $ winSet)
